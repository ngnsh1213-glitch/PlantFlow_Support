# AGENTS — PlantFlow_Support (Codex 진입 규칙)

> **[착수 전 필수] `pwd` 확인.** 작업 폴더가 `...\PlantFlow_Support`가 아니면(예: `PlantFlow_Ortho`) **즉시 중단**하고 "cwd 오배치: 현재 <경로>. PlantFlow_Support에서 다시 실행 필요"라고만 답한다. 규약 파일(`AGENTS.md`, `.plans/HANDOFF.md`)이 안 보이면 cwd 오배치를 의심한다.
> Codex는 이 파일을 읽는다. 운영수칙은 `CLAUDE.md`와 동일하게 유지한다.
> 둘 중 하나를 고치면 다른 하나도 함께 갱신한다.

## 프로젝트
- PlantFlow_Support (PFS): AutoCAD Plant3D 파이프 서포트 디테일 도면 자동 생성 (C#, .NET). PFO와 형제 제품군, 별도 리포.
- 원격: https://github.com/ngnsh1213-glitch/PlantFlow_Support (기본 브랜치 `master`)
- 이 리포 루트 = 프로젝트 루트(`.sln`·`.csproj`가 루트). Claude·Codex 공용.

## 상태 문서 (반드시 참조·갱신)
- `SESSION.md` — 현재 작업 상태·직전 실측·다음 분기. 세션 시작 시 먼저 읽는다.
- `TODO.md` — 잔여 백로그. 착수/완료 시 갱신.
- `CHANGELOG.md` — 변경 이력 요약. 의미 있는 변경 완료 시 추가.

## 핸드오프 왕복 규약 (Codex 진입점)
- 착수 시 **`.plans/HANDOFF.md`를 먼저 읽는다**. `status: ready`면 그 지시만 집도한다.
- 완료 후 **`.plans/REPORT.md`를 덮어써** 결과(변경 요약·파일·빌드결과·commit 해시)를 기록하고 코드를 커밋한다.
- 결과 diff 전문을 사용자 대화창에 붙여넣지 않는다(커밋으로 대체). 지시 범위 밖 수정 금지.
- **트리거 예약어**: 사용자가 메시지 전체로 **정확히 `1` 단독**(trim 후)을 보내면 "`.plans/HANDOFF.md` 읽고 집도" 지시로 해석한다. `1`이 다른 텍스트와 섞이면(`1번`, `step 1`) 일반 문장으로 처리(트리거 아님).

## 핵심 규칙
1. **집도 잠금**: 명시적 승인 없이 코드 수정 금지. 기본 모드는 분석.
2. 계획 → 승인 → 집도. 수정 후 변경 전후 재검증. **계측 우선, 맹목 apply 금지**.
3. 변경 사항은 리터럴 코드 블록(SEARCH/REPLACE)으로 명시.
4. 방어적 프로그래밍, 빈 catch 금지.
5. 파일 수정 후 커밋. push 전 `git status`로 민감 파일 제외 확인.
6. 모든 산출물 **한글**, 과장/비유 배제.
7. **빌드는 완료 조건이다.** `dotnet build` 오류 0을 확인하지 않았으면 커밋하지 말고
   `REPORT.md`에 `status: blocked`로 기록한다. "빌드 수동 원칙"은 예외 사유가 아니다.
   (2026-07-21 cycle98: 빌드를 건너뛴 채 커밋해 CS0103 8건으로 컴파일 불가 코드가 리포에 들어갔다.)
8. **push 금지.** Codex는 커밋까지만 한다. push는 사용자가 `push_support.bat`으로 수행한다.
9. **push된 커밋은 절대 amend·rebase하지 않는다.** 수정이 필요하면 새 커밋을 쌓는다.
   (2026-07-21: `1b55d76`을 push한 뒤 amend해 `3017b1d`이 되면서 원격과 이력이 갈라져
   merge 충돌이 발생했다. 8번을 지키면 amend를 해도 이 문제는 생기지 않는다.)
