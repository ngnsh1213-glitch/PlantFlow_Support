...
- [ ] **[통합] 일괄 추출 엔진 — 수집기 3종(다중선택 / 라인 / 타입)**: (구 아이디어 3건 ②다중선택·⑥라인단위·⑦타입단위를 1건으로 통합 — 2026-07-24. 본문이 이미 "동일 필터·추출 파이프라인 공유, 수집기만 교체"로 명시해 중복이었음.)
  여러 서포트를 잡아도 **서포트 본체만 필터링**(유볼트·슈·TR 등 부속 제외)하여 각 서포트마다 **개별 도면**으로 추출. 부속은 세트 규칙에 따라 해당 서포트 도면에 자동 포함(기존 AutoIncludeRelatedParts 재사용).
  - **핵심 구조**: 필터·추출 파이프라인을 먼저 만들고 **선택 수집기만 갈아끼운다**.
    - 수집기 A = **수동 다중선택**(영역/개별 픽).
    - 수집기 B = **라인 단위**(Plant3D `Add to Selection > Connected Line Number`. 배관 1개 클릭→연결 아이템 순회→서포트만 추출). API 재현=포트 그래프 순회(P5 토폴로지 식별 자산 재사용) vs 기존 Plant 명령 호출.
    - 수집기 C = **타입 질의**(GD1/RS/FS… 지정→범위 내 전량). 범위=현재 도면 vs 프로젝트 전역.
  - 공통 쟁점: 서포트 본체 vs 부속 판별(SupportType/RowClassName), 제외 부속 목록의 **설정화**(하드코딩 금지), 추출 순서·태그 정렬, 도면 파일 분리 vs 다중 레이아웃, **실패 1건이 전체를 중단시키지 않도록 건별 예외 처리**, 대량 시 진행률·중단 수단.
...
### 후순위 강등 (필요성·비용 재고 — 2026-07-24)

- [ ] **[후순위] 서포트 생성 트랙 (그림 클릭 생성 탭 + 세트 자동배치)**: (구 ③ 신규 생성 탭에 구 ① 세트를 흡수 — 2026-07-24. **강등 사유**: PFS 핵심 가치는 *추출·도면화*이지 생성 UI가 아니며, 사용자가 Plant3D SPEC 뷰어에 이미 익숙함. 썸네일 원천 확보 비용 대비 차별화 기여 낮음.)
  - **생성 탭**: PFS UI에 "서포트 생성" 탭 추가, 타입별 **그림(썸네일) 클릭→모델에 생성**(SPEC 뷰어 우회). 구성=카탈로그(.acat) 타입 목록→썸네일 그리드→배치 명령(파이프 선택→BOP/사이즈 자동 산출).
    - 쟁점: 썸네일 원천(규격집 PDF 발췌 vs 렌더), 카탈로그 타입 동적 로딩, WebView(app.html) 라우팅 추가, 배치 명령 컨텍스트(모드리스→ExecuteInCommandContextAsync) 유예 재발 방지.
  - **세트(SET) 자동배치** (구 ①): GD1만 눌러도 UBOLT 동반 생성. 두 객체 각각 **독립 Plant 파트 유지**(BOM/태그 정상)하되 부모 태그(`GD1-001`)로 그룹핑→무탭 추출 시 한 세트 취급. 위치·사이즈=held-pipe BOP 매칭 재사용.
    - 선결 규칙 4건: ①타입별 UBOLT 동반 여부 ②파이프 사이즈→UBOLT 규격 매핑표 ③개수(1/2개) ④제외 옵션.
    - 대안 검토됨(모두 부적합): Python 콘텐츠 동반작도(BOM 개별행 불가), .acat Assembly 등록(.acat+.pspc 이중관리 비용 큼).

- [ ] **[후순위·게이트 선결] 서포트 생성 후 "Viewer Open" → 나비스웍스 관측점 이동**: PFS에서 서포트 생성 후 "Viewer Open"→나비스웍스가 열리며 **클릭한 서포트 위치로 뷰포인트 자동 이동**.
  - 전제 조건: `<프로젝트 폴더>\Plant 3D Models`에 각 모델(.dwg)과 **동명의 `.nwc`** 존재 + 합친 **`.nwf`**로 이동, nwf 열고 서포트로 뷰포인트 이동.
  - **선결 게이트(강등 사유)**: nwc/nwf가 **최신인지 자동 확인·재변환 안내가 없으면 좌표는 맞아도 형상이 어긋나 실현 무의미**. 현장에서 nwc 최신성이 잘 안 지켜짐 → 이 게이트를 먼저 설계해야 착수 가치 발생. 나비스웍스 API 제약=삭제·지오메트리 수정 불가(관측점·숨김·오버라이드·세트만).
  - 기반=[[navisworks-plantflow-integration-idea]] NavisHidePick 파이프라인 검증 완료(C:\Lisp\NavisHidePick). 후보 ②(도면↔모델 관측점 왕복)의 구체화.
  - 쟁점: ①PFS↔나비스웍스 프로세스 간 통신(파일 기반 관측점 vs Automation API vs COM) ②서포트 좌표→카메라 관측점 변환 ③선택·하이라이트까지 vs 카메라 이동만 ④나비스웍스 미실행 시 자동 기동.
