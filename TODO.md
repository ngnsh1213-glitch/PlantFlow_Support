...
- [x] **무탭 타 타입 회귀 확인 완료**(cycle105~116, 2026-07-23): GD1~3+RC1~9 전 타입 통과 → CHANGELOG.
      RC 전제 검증 결과=①F1/F2 매핑은 타입별 상이(RC5는 S2=가로재/S3=세로재 — 원본 .py 대조 필수) ②S2 포트 전제는 RC1~3 한정
      ③타입별 config 행 추가가 설계 관용구. 잔여=RC4 눈확인 1회(수정 후 명시 판정 없었음, 배치 추출 시).
- [ ] ~~(구) 무탭 타 타입 회귀 확인~~: `PFSNOTABBATCH` 전량 추출로
