...
### ④ 다음 작업 — 라인넘버(파이프) 콜아웃 꺾임 리더 **(착수 전)**
cycle 100에서 공통 경로를 1자로 통일하면서 파이프 콜아웃까지 직선이 됐다.
사용자 요구: **파이프만 꺾임 1회**(경사 + 수평 꼬리), 유볼트는 1자 유지.
자문이 제시한 불변식:
| 단계 | 유볼트 | 파이프 |
|---|---|---|
...
| 작도 | 같은 1선분 | 같은 2선분 |
| 등록 | 같은 1선분 | 같은 2선분 |
`Commit`의 3점 오버로드가 이미 두 선분 등록을 지원한다. cycle 100의 문자 중단 접속과
충돌하지 않는다 — 파이프의 `textEdge`는 그대로 두고 앞에 `elbow`만 넣는다.
