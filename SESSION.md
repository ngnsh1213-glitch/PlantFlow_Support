...
## cycle 92 집도 완료 — 라이브 검증 대기 (2026-07-20)

- RC 세로 치수와 세로 부재 콜아웃은 `S2` 포트(index=1·이름 일치) + F2 높이의 기둥 기준으로 작도한다. 포트·F2·투영·범위 가드 실패 시 기존 기준으로 폴백하며 원인을 로그에 남긴다.
- U-bolt는 `Tag`와 원본 bbox 중심을 별도 스냅샷하여 개별 콜아웃으로 작도한다. 빈/중복 태그와 `ShortDescription != UB`는 제외한다.
- 사용자 수동 `dev_test.bat` 후 RC1/RC2/RC3 치수·기둥 앵커, RC1의 UB-002/UB-003 두 콜아웃, GD1~GD3 회귀를 확인할 것.
