...
## ★★★ 진행 중 — 무탭 2D 평면화 트랙 (cycle 122~127, 2026-07-24)
...
### 사용자 추가 요구 — 객체 블럭화 (Phase 1 코드 완료, 라이브 검증 대기)
...
- cycle127 `5c9cb25`(Claude 빌드 재확인 오류0/경고14): 솔리드 출처 분류 후 support/pipe/유볼트 tag별 BlockTableRecord와 공유 기준점 BlockReference 생성.

### ★cycle127 라이브 결과 — 블럭 동작하나 분류 결함 확정 (2026-07-24)
- **블럭 생성 동작**(형상 유실 0). Claude가 지적한 UpgradeOpen 결함은 **미발현**(AutoCAD 관용, 예측 빗나감).
- **★분류 결함(로그)**: `PFSNOTABBLOCK support=0 pipe=0 ubolt=UB-003,UB-002 blocks=2` — F1/F2 부재가 전부 유볼트 블럭에 흡수. 근인=조건 "유볼트 중심이 솔리드 확장 extents 내부"가 헐렁 → 유볼트가 베이스플레이트 위에 앉아 그 중심이 큰 베이스플레이트 extents 안에 듦 → 오분류. tol(대각선×0.25) 큰 부재엔 과대.
- **수정 방향**: 분류 뒤집기 = "유볼트 tag마다 중심 최근접 솔리드 1개만 유볼트". 큰 부재는 중심 멀어 support 잔류.

### Phase 2 = 카테고리별 렌더링 (사용자 확정 2026-07-24)
- **렌더링 매핑 확정**: 서포트(각재)=`Brep.Edges` 위상에지 투영(현행 깨끗) / **배관=히든처리** / **유볼트=히든처리**(별도 = 실루엣/심볼 후 블럭 합침).
- **배관 히든처리**: Main뷰=파이프축 정면이라 원기둥=깨끗한 원 1개. `s_isoPipeCenterWcs`+`s_isoPipeRadiusModel`로 페이퍼에 원 직접 작도(해석적, 캘리브레이션 불요). 멀티뷰 확장 시 옆면 실루엣 2선 필요.
- **유볼트 히든처리**: 정면 뷰라 실루엣=다리 평행선2+아치 동심호2 → 승환님 참조 이미지와 일치(반원 아치·봉두께 2선·수직다리·수평바·워셔너트). 방식(실루엣② vs 파라메트릭 심볼①)은 **스파이크로 판정**.
- **★스파이크 먼저**(사용자 지시 2026-07-24, cycle128): `Brep.Faces` 표면타입/파라미터(원기둥 축·반경, 토러스 중심·반경) 로그 덤프 + **배관 원 렌더 출처 계측**(투영 솔리드 vs 주석). 작도 없음. 관리형 API가 곡면 파라미터 열면→②, 아니면→①.
- 전제 수정: **분류 뒤집기**(cycle127 결함) = "유볼트 tag마다 중심 최근접 솔리드 1개만 유볼트". Phase 2 본 집도에 포함.

### ★cycle128 스파이크 판정 = PASS → ②실 실루엣 확정 (2026-07-24, 로그 실측)
- Codex `ad890ae`, Claude 빌드 재확인 오류0/경고14. `PFSNOTABFACE` 로그.
- **관리형 API 곡면 파라미터 완전 노출**: Cylinder{Origin,Radius}, Torus{Center,MajorRadius,MinorRadius}, Plane{Normal} 모두 "판별=가능".
- **유볼트 실형상**: 35면 = 토러스 아치(MajorR52·MinorR6) + 다리 원기둥(R6) + 육각너트(Normal 30° 평면) + 워셔(R12). Main뷰=파이프축(0,1,0) 정면, 유볼트=X-Z평면 → 실루엣=다리 평행선+아치 동심호2 = 참조 이미지 일치. **자동 산출→캘리브레이션 불요.**
- **★배관 미스터리 해소**: 배관 솔리드 **존재**(`...672`=Cylinder R44.55≈파이프 44.45, axis(0,1,0)). cycle127 결함이 UB-002로 오분류→"pipe=0"였음. 도면 100A 원=이 원기둥 **끝단 원 에지**(축정면이라 깨끗). **Main뷰선 배관 에지투영만으로 이미 깔끔**(특별처리 불요, 멀티뷰서 옆면만 실루엣 필요).
- **렌더링 매핑(Main)**: 서포트=Brep.Edges / 배관=Brep.Edges(끝단 원, 이미 깨끗) / **유볼트=②실 실루엣(유일 신규작업)**.

### 다음 = Phase 2 본집도 (`3`으로 계획→자문→핸드오프)
- ①**분류 뒤집기**(전제): 유볼트 tag별 중심 최근접 1솔리드=ubolt / R44.55 원기둥=pipe / 나머지=support. (배관 오분류 교정)
- ②**유볼트 실루엣 렌더**: 곡면 해석적 실루엣 — 원기둥 축·반경→평행선, 토러스→동심호, 육각/워셔 에지. **Main뷰(face-on) 우선**, 멀티뷰 실루엣은 후속(토러스 일반 실루엣 난이도↑).
- ③블럭 재정합: support/pipe/ub tag별 올바른 분리.
- 자문(§9) 수행 — 실루엣 계산이 신규 복잡도라 Codex 자문 필요.
...
- **격리**(`57e0d20`): AutoIncludeRelatedParts가 접촉박스 안 Support를 무구분 포함하던 결함 —
  부속 코드 단일 원천(`GetNotabAttachmentCodes`, BATCH 공용) + 본체 skip(main-support) + 빈값 포함+WARN.
- **부속 단독 추출 거부**(`f6c3c4a`): 파이프라인 진입 가드 — 본체 0+부속만이면 도면 미생성+메시지.
- **리더 관통 타입별 0 확산**: RC1/2/3/6/9(`e3cdfe3`·`98a8320`·`3c167f2`) — 20 유지는 RC4·RC5만(사용자 확정).
- 검증 전건 통과: RC1 혼입 소멸 / UB-020 거부 / RC6·RC9 화살촉 정착 / 정상 추출 무간섭.
- **전체 현황: GD1~3+RC1~9+RS1~15(변형 18종) + 격리·가드 종결.** 잔여 = TR/TRS·FS·SHOE 검수, push 대기.
...
| 서포트 | F1(가로재) | F2(세로재) | P1 |
|---|---|---|---|
| RC1 | `k=0` dist=19.6 (우) | `k=3` dist=48.4 (좌) | 정상 |
| RC2 | `k=0` dist=19.6 (우) | `k=3` dist=48.4 (좌) | 정상 |
| RC3 | `k=0` dist=19.6 (우) | `k=4` dist=58 (우) | 정상 |

### ② 확정 배치 규칙 (수작업 도면 3장 대조. 재논의 금지)
1. **부재 밸룬은 그 부재의 "여유 있는 쪽 끝단" 바로 바깥.** 화살표는 그 끝단에 닿고,
   리더는 **짧은 수평 직선 1개**. 좌·우 두 후보를 같은 단계에서 평가해 여유 큰 쪽 채택(동점=우측).
2. **유볼트 화살표는 좌/우 세로 변 중점만.** U자 하변 중점은 다리 사이 빈 공간이라 금지.
   리더는 꺾임 없는 직선 1개, 문자 **중단**에 접속, 이격 `PFS_NOTAB_UBOLT_MIN_DX`(기본 10).
3. **실패 시 폴백 없이 작도 생략(R2).** 잘못된 방향의 밸룬보다 없는 편이 낫다.
4. F1=가로재, F2=세로재, P1=플레이트(포트 그대로).

### ③ 재조사 금지 실측 (반복 확인 완료)
- **세로 기둥은 독립 솔리드가 아니다.** `member-spike` 결과 솔리드 4개 중 `7A`가
  기둥+가로재+플레이트 병합(paperWH 90×119.5), 나머지는 유볼트·볼트.
  → 세로재 좌표는 **솔리드 상자가 아니라 포트 기반**(`rawAnchor`=S2 투영, `verticalBaseY/TopY`=S2+F2×vScale).
- **`verticalX`는 치수선 X이지 부재 X가 아니다.** 밸룬 형상 좌표로 쓰면 안 된다.
- **`IsBalloonFree`는 리더-장애물 교차를 검사하지 않는다.** 밸룬 **원 상자**만 본다.
  따라서 긴 F2 리더가 서포트·치수선 위를 지나는 것은 **허용된 동작**이다.
  이를 막으면 F2는 대부분 skip된다(짧은 리더와 무교차를 동시에 만족할 공간이 없다).
- 기둥 축이 서포트 실루엣 한가운데라 고정 거리로는 좌우 모두 막힌다.
  자문 계산 필요 단계: 축 292.5→우8/좌0, 311→우6/좌1, 342→우3/좌4. 상한 기본 12.

### ④ 라인넘버(파이프) 꺾임 리더 — **종결** (`152dc46`)
사용자 판정 "이상없음". 라이브 3회 `callout-skip`·`balloon-skip` **0건**, 파이프 전부 `tier=0`
(가장 엄격한 리더 교차 검사 단계에서 통과).
```
RC1 elbow=(384.5,355.72)  tail=16 endX=400.5  tier=0 scanned=723
RC2 elbow=(370.77,304.59) tail=16 endX=386.77 tier=0 scanned=639
RC3 elbow=(384.06,350.54) tail=16 endX=400.06 tier=0 scanned=34
UB-002/003/004/006 elbow=none tail=0   ← 직선 유지
```
- `TryPlace(… tailLength …)`: >0이면 문자 접속점에서 앵커 쪽으로 물러난 점이 꺾임점(`p1`),
  문자 접속점이 `p2`. 0이면 `p1==p2` 직선. **파이프만** `label=="pipe callout"`으로 꼬리를 받는다.
- 검사 `Free`는 원래부터 `anchor→p1`, `p1→p2` 두 선분을 본다. 직선일 때 둘째가 퇴화였을 뿐이라
  꺾임을 넣자 자동으로 두 구간이 검사된다. 작도·등록(`Commit` 3점)도 같은 2선분을 쓴다.
- 노브 `PFS_NOTAB_PIPE_LEADER_TAIL` 기본 `txt×2`=16. **0이면 직선으로 되돌아간다.**
- ⚠ 관찰: RC3 파이프가 `scanned=34`로 유효 후보가 다른 건(639~723)의 1/20이다.
  형상이 복잡해지면 이 케이스가 먼저 skip으로 떨어질 수 있다.
  그때는 `PFS_NOTAB_PIPE_LEADER_TAIL`을 줄이면 후보가 늘어난다(재빌드 불필요).

### ④-1 다음 작업 — **미정**
무탭 주석 트랙(치수·콜아웃·밸룬·BOM)은 RC1~RC3 기준으로 일단락됐다.
후보: ①GD/FS 등 타 타입 회귀 확인(`PFSNOTABBATCH` 전량 추출) ②`TODO.md`의 서포트 세트(SET)·
태그 자동채번 ③핫리로드 제품 통합.

### ⑤ 프로세스 교훈
- **cycle 98 사고**: Codex가 "빌드 수동 원칙"을 이유로 `dotnet build`를 건너뛰어
  **컴파일 불가 코드(CS0103 8건)가 커밋**됐다. 이후 핸드오프에 "빌드 미실행 시 커밋 금지,
  `status: blocked` 반려"를 상단 고정 문구로 넣었다. Claude는 REPORT 수령 시 **직접 빌드 재확인**한다.
- **가설 3건이 자문으로 기각**됐다(리더 관통설, "박스 오채택은 무해", "port 전체 제외").
  코드 리딩 자문(Codex)이 라이브 재시도보다 싸다 — §1 3-스트라이크 규칙대로 조기 전환할 것.

### 관련 커밋 (cycle 97~102)
`8cfd311`(유볼트 변 중점) → `5383da6`(실측 박스) → `e2d0606`+`005d730`(앵커 전환+빌드수리)
→ `3017b1d`(1자 리더) → `1f3a276`(2단계 탐색) → `54d84fa`(local-box 제거)
→ `d11f6e4`(끝단 전용 배치) → `828d7bc`(F2 포트 기반) → `947663e`(수평 단계 확장)

---

## 이관 스레드 상태 (2026-07-21 오전, **①~⑥은 위 최신 블록으로 대체됨 — 이력 보존용**)
