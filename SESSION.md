...
### Phase 2 = 유볼트 표준 심볼 (사용자 확정)
- 승환님 확정(2026-07-24): **아치+다리+너트 파라메트릭**. ★참조 이미지 제공 = 반원 아치(∩, 봉 두께 2선)+수직 다리 2개+수평 바(부재 관통)+하단 워셔·너트. 이게 목표 도형(정답지).
- 핸드오프 범위: ①분류 뒤집기(전제) ②유볼트 솔리드 BRep 투영 제외 ③파라메트릭 심볼 작도(첫 컷+env 노브, [feedback-manual-calibration-workflow]) ④심볼을 `PFS_FLAT_UB_<tag>` 블럭 주입. 각재/배관 블럭 올바르게 분리.
- ★치수/비율/방향은 유볼트·파이프 크기에서 산출 후 env 노브로 캘리브레이션. HANTEC 규격집 대조.

### 다음 결정 분기
- **Phase 2 핸드오프**: `3`으로 계획→자문→핸드오프 발행 대기(분류 뒤집기 + 참조 이미지 기반 심볼).
