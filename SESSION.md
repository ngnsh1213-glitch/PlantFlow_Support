...
- cycle127 `5c9cb25`: 솔리드 출처 분류 후 support/pipe/유볼트 tag별 BlockTableRecord와 공유 기준점 BlockReference를 생성한다. `PFSNOTABBLOCK` 로그와 RC1-001 라이브 검증이 남아 있다.

### 다음 결정 분기
- **유볼트 서브트랙**: Phase 1 블럭화 라이브 검증 후 1안(표준 2D 심볼)으로 진행. [pfs-hantec-support-standard-catalog] 참조.
