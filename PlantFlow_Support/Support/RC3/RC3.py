...
from support_data import pipe_size_JIS as pipe_size, profile, baseplate_RC3 as baseplate
...
def RC3(s, Dn = 100, BI = 210, A = 250.0, A1 = 250.0, A2 = 250.0,
  F2 = 600.0, P1 = 0.0, ID = "RC3", **kwargs):
  """
  """
  # Check member profile as standard
  valid_member = [15, 16, 17, 210, 212, 215, 312, 315, 320]
  if (BI not in valid_member):
    e1 = ERROR_SHAPE(s, R = 100.0)
    print("Please input valid member profile!")
    print("BI: 15, 16, 17, 210, 212, 215, 312, 315, 320")
    return
  ...
  if (BPn == 1):
    ...
    pl1.translate((A, -T1 - Fa1/2, plt/2 - F2))
    m1.uniteWith(pl1)
    pl1.erase()

    sP2 = (A - A2 - Fa1/2, -T1, -F2 - Dz/2 - P1)
    sP3 = (-A2, 0.0, -Dz/2 - P1)
    sP4 = (A - A2 + plA/2, 0.0, -Dz/2 - P1 - F2)
    ...
    pl1.translate((A, -Fa2/2, -F2 - Fa1 + plt/2))
    m1.uniteWith(pl1)
    pl1.erase()

    sP2 = (A - A2, 0.0, -F2 - Dz/2 - P1 - Fa1)
    sP3 = (-A2, 0.0, -Dz/2 - P1)
    sP4 = (A - A2 + plA/2, 0.0, -Dz/2 - P1 - F2 - Fa1)
    ...
    m2.translate((A, 0.0, -F2 - Fa1))
    m1.uniteWith(m2)
    m2.erase()

    pl1.translate((A, 0.0, -F2 - Fa1 + plt/2))
    m1.uniteWith(pl1)
    pl1.erase()
