...
def UBD(s, Dn = 50, C = 650.0, L = 500.0, S = 190.0, R = 308.8, 
  B = 120.0, SN = 0.0, TY = 0, ID = "UBD", **kwargs):
  """
  """
  ...
  b1.translate((-psize/2 - 1.5, 0.0, 0.0))
  o1.uniteWith(b1)
  b1.erase()
  ...
  n3 = HEXAGON(s, R = nS/2, H = nH)
  n4 = HEXAGON(s, R = nS/2, H = nH)
  ...
  w3 = CYLINDER(s, R = wD/2, H = wH, O = 0.0)
  w3.translate((0.0, 0.0, nH))
  n3.uniteWith(w3)
  w3.erase()
  n3.rotateY(-90).translate((-psize/2 + wH + nH, R + d/2, 0.0))

  w4 = CYLINDER(s, R = wD/2, H = wH, O = 0.0)
  w4.translate((0.0, 0.0, nH))
  n4.uniteWith(w4)
  w4.erase()
  n4.rotateY(-90).translate((-psize/2 + wH + nH, -R - d/2, 0.0))
  ...
  if (TY in [-1, -2, -3]):
    if (Dn > 100):
      e1 = ERROR_SHAPE(s, R = 50.0)
      print("Error: Dn > 100 is not supported for this type.")
      return

  if (TY == -1):
    fr1 = L_SHAPE(s, BF = 5, LE = B)
    fr1.rotateX(-90).rotateY(180).translate((-psize/2 - 3, -B/2, -30.0))
    o1.uniteWith(fr1)
    fr1.erase()
    ...
    fr1.translate((-psize/2 - 3 - frT/2, 0.0, 0.0))
    o1.uniteWith(fr1)
    fr1.erase()
