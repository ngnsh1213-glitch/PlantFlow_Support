...
@param(B1=LENGTH, TooltipLong="B param")
@param(P1=LENGTH, TooltipLong="P1 length")
@param(TY=LENGTH, TooltipLong="|TYPE A = -1|TYPE B = -2|")

def RC5(s, Dn = 100, BI = 15, A = 400.0, A1 = 250.0, 
  F2 = 600.0, B1 = 0.0, P1 = 0.0, TY = -1, ID = "RC5", **kwargs):
  """
  """
  ...
  if (TY == -1):
    ...
    m2.rotateZ(180).translate((Fa1 + 10.0, 0.0, 0.0))

    pl1.rotateX(90).translate((F1 - plA/4, plt/2, -Fa1/2))
    pl2.rotateX(90).translate((Fa1/2 + 10.0, plt/2, F2 - plA/4))
    m1.uniteWith(m2)
    m1.uniteWith(pl1)
    m1.uniteWith(pl2)
    m2.erase()
    ...
    sP2 = (A, 0.0, -Dz/2 - P1)
    sP3 = (10.0 - A1, 0.0, F2 - Dz/2 - P1)
    sP4 = (A + plA/4, 0.0, -Dz/2 - P1)
    sP5 = (10.0 - A1, 0.0, F2 - Dz/2 - P1 + plA/4)
    sLD = [(Fa1 + 10.0 - A1, 0.0, - Dz/2 - P1), 
           (Fa1 + 10.0 - A1, 0.0, F2 - Dz/2 - P1)]

    s.setLinearDimension('A', (0.0, 0.0, -Dz/2 - P1 - Fa1), 
                              (A, 0.0, -Dz/2 - P1 - Fa1))


  elif (TY == -2):
    if (BI not in [210, 212, 215]):
      pl1.erase()
      pl2.erase()
      e1 = ERROR_SHAPE(s, R = 100.0)
      print("Only C10, C12, C15 for TYPE-B!")
      return
    ...
    m2.rotateZ(-90).translate((Fa1/2 + 10.0, 0.0, 0.0))
    pl1.rotateX(90).translate((F1 - plA/4, plt/2, -Fa1/2))
    pl2.rotateX(90).translate((Fa1/2 + 10.0, plt/2, F2 - plA/4))
    ...
    sP3 = (10.0 - A1, 0.0, F2 - Dz/2 - P1)
    sP4 = (A + plA/4, 0.0, -Dz/2 - P1 - Fa1/2)
    sP5 = (10.0 - A1, 0.0, F2 + plA/4 - Dz/2 - P1)
    sLD = [(Fa1/2 + 10.0 - A1, 0.0, -Dz/2 - P1), 
           (Fa1/2 + 10.0 - A1, 0.0, F2 - Dz/2 - P1)]
