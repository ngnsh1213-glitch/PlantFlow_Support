...
def RS3(s, Dn = 100, BI = 210, A = 600.0, A1 = 200.0, F2 = 500.0, P1 = 0.0,
  TY = -2, ID = "RS3", **kwargs):
  """
  """
  ...
  if (TY not in [-1, -2, -3, -4, -5, -6]):
    e1 = ERROR_SHAPE(s, R = 100.0)
    print("Please input valid type!")
    print("TY: -1, -2, -3, -4, -5, -6")
    return
  ...
  if (BPn == 1):
    Fa1 = profile[BI]["F"]
    T1 = profile[BI]["T"]
    m1 = L_SHAPE(s, BF = BF, LE = F2 - T1)
    m2 = L_SHAPE(s, BF = BF, LE = F1)
    if (TY in [-1, -3, -5, -6]):
      m2.rotateY(90).translate((0.0, -T1, F2))
      sP1 = (Fa1, 0.0, F2)
      lP1 = [(0.0, 0.0, -Dz/2 - P1), (-A1, 0.0, -Dz/2 - P1)]
      lP2 = [(0.0, 0.0, -Dz/2 - P1), (A, 0.0, -Dz/2 - P1)]

    elif (TY in [-2, -4]):
      m2.rotateZ(-90).rotateY(90).translate((0.0, 0.0, F2))
      sP1 = (Fa1, 0.0, F2)
      lP1 = [(0.0, 0.0, -Dz/2 - P1), (-A1, 0.0, -Dz/2 - P1)]
      lP2 = [(0.0, 0.0, -Dz/2 - P1), (A, 0.0, -Dz/2 - P1)]
    ...
    sP2 = (-A1, 0.0, -F2 - Dz/2 - P1)
    sP3 = (A, -Fa1, - Dz/2 - P1)
    sPL = [(-A1, Fa1, -Dz/2 - P1), (-A1, Fa1, -F2 - Dz/2 - P1)]
    ...
    if (TY in [-1, -3, -5, -6]):
      ...
      sP2 = (-A1 - Fa1/2, 0.0, -F2 - Dz/2 - P1)
      sP3 = (A, Fa2, -Dz/2 - P1)
      sPL = [(-A1, 0.0, -Dz/2 - P1), (-A1, 0.0, -F2 - Dz/2 - P1)]
  ...
  if (BPn == 1):
    m1.translate((-A1, 0.0, -F2 - Dz/2 - P1))
  elif (BPn == 2 or BPn == 3):
    if (TY in [-1, -3, -5, -6]):
      m1.translate((-A1 - Fa1/2, 0.0, -F2 - Dz/2 - P1))
    elif (TY in [-2, -4]):
      m1.translate((-A1 + Fa1/2, 0.0, -F2 - Dz/2 - P1))
